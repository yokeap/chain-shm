...
wear_analysis.py
================
1D wear measurement using (7)→(8) and (9)→(10).

(7)→(8): vertical chain thickness at side arc (yellow line, already from side_arc.py)
(9)→(10): horizontal wire edges at x = circle center (red line)
  (9)  = wire top edge at x_circle_center
  (10) = wire bot edge at x_circle_center

wear is computed from how much (9)→(10) overlaps with (7)→(8).
...
def _wire_y_at_x(slope, intercept, x):
...
def _build_crescent_mask(side_arc, wire_seg, shape, full_mask=None):
    ...
    mo = (outer_pts[:,1] >= ywt_o-5) & (outer_pts[:,1] <= ywb_o+5)
    occ = outer_pts[mo]
    ywt_i = s_top*inner_pts[:,0]+i_top; ywb_i = s_bot*inner_pts[:,0]+i_bot
    mi = (inner_pts[:,1] >= ywt_i-5) & (inner_pts[:,1] <= ywb_i+5)
    icc = inner_pts[mi]
...
# ══════════════════════════════════════════════════════════════════
# Compute wear (1D: remaining thickness method)
# ══════════════════════════════════════════════════════════════════
...
def compute_wear(recon_results, hw_result, image_shape=None, vert_mask=None):
    ...
    for sa in side_arcs:
        ...
        # (7)→(8): d_side (vertical chain thickness at side)
        d_side = sa["d_side_px"]
        pt7 = sa["pt7"]
        pt8 = sa["pt8"]

        # (9): wire top edge at x = circle center
        # (10): wire bot edge at x = circle center
        y9 = _wire_y_at_x(best["slope_top"], best["intercept_top"], cx_c)
        y10 = _wire_y_at_x(best["slope_bot"], best["intercept_bot"], cx_c)
        pt9 = (cx_c, y9)
        pt10 = (cx_c, y10)
        b_at_contact = abs(y10 - y9)   # wire thickness at this x

        # x_tool: circle edge facing link center
        if side == "left":
            x_tool = cx_c + r_c
        else:
            x_tool = cx_c - r_c

        # x_orig: outer side-arc at y = midpoint of (7)(8)
        y_measure = (pt7[1] + pt8[1]) / 2.0
        x_orig = _find_x_on_curve_nearest(sa["outer_pts"], y_measure, cx_c)
        if x_orig is None: continue

        # remaining = |x_tool − x_orig| = distance from outer arc to circle edge
        remaining = abs(x_tool - x_orig)

        # measured_depth = d_side − remaining (how much circle eats into d_side)
        ...
        logger.info(
            f"  Wear {side}: d_side={d_side:.0f}  remaining={remaining:.0f}  "
            f"measured={measured_depth:.1f}  wear={wear_pct:.1f}%  "
            f"b_at_contact={b_at_contact:.0f}")
...
def draw_full_recon(image, recon_results, hw_result, wear_result, vert_mask=None):
    ...
    # 8. (9)→(10) red line + Δw dimension + wear labels
    for p in wear_result.get("pairs",[]):
        pt9, pt10 = p["pt9"], p["pt10"]
        ym = int(p["y_measure"])
        xt, xo = p["x_tool"], p["x_orig"]
        md, wp, side = p["measured_depth"], p["wear_pct"], p["side"]

        # Draw (9)→(10): red vertical line (horizontal wire at contact)
        cv2.line(vis, (int(pt9[0]),int(pt9[1])), (int(pt10[0]),int(pt10[1])),
                 RED, 3, cv2.LINE_AA)
        _dot(vis, pt9, RED, 6, "9")
        _dot(vis, pt10, RED, 6, "10")

        # Δw arrow from x_orig to x_tool at y_measure
        cv2.arrowedLine(vis,(int(xo),ym),(int(xt),ym),
                        DARK_RED,2,cv2.LINE_AA,tipLength=0.1)

        # Label
        if side == "left":
            lx = min(int(xt),int(xo)) - 10
            cv2.putText(vis,f"wear={wp:.1f}%",(lx-130,ym-12),
                        cv2.FONT_HERSHEY_SIMPLEX,0.6,MAGENTA,2,cv2.LINE_AA)
            cv2.putText(vis,f"Dw={md:.0f}px",(lx-130,ym+12),
                        cv2.FONT_HERSHEY_SIMPLEX,0.5,MAGENTA,1,cv2.LINE_AA)
        else:
            lx = max(int(xt),int(xo)) + 10
            cv2.putText(vis,f"wear={wp:.1f}%",(lx,ym-12),
                        cv2.FONT_HERSHEY_SIMPLEX,0.6,MAGENTA,2,cv2.LINE_AA)
            cv2.putText(vis,f"Dw={md:.0f}px",(lx,ym+12),
                        cv2.FONT_HERSHEY_SIMPLEX,0.5,MAGENTA,1,cv2.LINE_AA)
